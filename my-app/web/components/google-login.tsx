...
function GoogleSignIn() {
    ...
    useEffect(() => {
        if (session?.ServerResponse && session?.ServerResponse.user && session?.ServerResponse.token) {
            const {user, token} = session?.ServerResponse
            dispatch(setUserInfo({...user, accessToken: token.access_token, refreshToken: token.refresh_token}))
        }
    }, [session]);
